...
| # | 内容 | 受け入れ基準(機械検証) |
|---|---|---|
| M1 | `happy-client`: 暗号化・デバイスリンク単体実装 | このMacをuser-scopedデバイスとしてリンクでき、既存セッションのメタデータを実際に復号できる(スクリプト/テストで確認) |
| M2 | 読み取り専用: 1台分のセッション一覧+ライブ表示 | 実際に動いているMac上のセッションのテキストがリアルタイムに表示される |
| M3 | 4台横断のグリッド + タブ(ワークスペース) | 4台のセッションが1画面に同時表示され、タブでグルーピングできる |
| M4 | 操作系: メッセージ送信・permission/model/reasoning切替・allow/deny・abort/kill・新規セッション起動 | 各操作が実機のセッションに反映される |
| M5 | 一括操作(複数選択→一括送信・一括承認) | 複数セッションへの一括操作が動作する |
| M6 | Macネイティブ通知 | セッション完了等で通知が飛ぶ |
| M7 | デザイン仕上げ | 見た目レビュー(人間ゲート) |
